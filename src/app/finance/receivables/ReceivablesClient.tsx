...
export default function ReceivablesClient({ invoices }: { invoices: any[] }) {
  const columns: Column[] = [
    { key: 'invoiceNo', label: 'Invoice No', render: (r) => <span style={{ fontFamily: 'monospace', fontWeight: 600 }}>{r.invoiceNo}</span> },
    { key: 'customer', label: 'Customer', render: (r) => <div style={{ fontWeight: 600 }}>{r.customer}</div> },
    ...
    { key: 'status', label: 'Status', render: (r) => {
      ...
      if (r.status === 'Sent') { bg = 'rgba(245, 158, 11, 0.1)'; color = 'var(--warning)'; }
  ...
  return (
    <div style={{ background: 'var(--bg-secondary)', borderRadius: '12px', border: '1px solid var(--border-color)', overflow: 'hidden' }}>
      <EnterpriseDataTable columns={columns} data={invoices} searchPlaceholder="Search by invoice number or customer..." />
    </div>
  );
}
